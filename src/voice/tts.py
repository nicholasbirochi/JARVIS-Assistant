...
process are both available (see voice/xtts_client.py and
...
from the worker (voice/xtts_client.py's synthesize_stream()) rather
...
from config import TTS_ENGINE, TTS_VOICE
...
def _speak_via_xtts(text: str, stop_event: threading.Event | None) -> bool:
    ...
    from voice.xtts_engine import STREAM_SAMPLE_RATE
...
def speak(text: str, stop_event: threading.Event | None = None) -> None:
    ...
    it's plain textContent, see visualizer/page.html), but
    ...
    from voice.speech_text import spell_out_acronyms, strip_markdown
