...
def test_run_active_session_speaks_goodbye_and_unloads_on_stop_phrase(monkeypatch):
    unload_calls = []
    monkeypatch.setattr("voice.stt.unload", lambda: unload_calls.append(1))
...
def test_run_active_session_publishes_thinking_between_transcription_and_llm_call(monkeypatch):
    ...
    monkeypatch.setattr("voice.stt.unload", lambda: None)
    published: list[str] = []
...
def test_run_voice_loop_goes_straight_to_active_session_without_speaking_first(monkeypatch):
    ...
    monkeypatch.setattr("voice.xtts_client.ensure_worker_started", lambda: None)
    speak_calls = []
    monkeypatch.setattr("voice.tts.speak", lambda text, stop_event=None: speak_calls.append(text))
...
def test_run_voice_loop_forwards_wake_event_to_listener_wait(monkeypatch):
    # menubar.py sets this from a real NSWorkspace wake notification
