...
from resume.schema import Conflict


def _reviewed_dir() -> Path:
    from config import PROPOSALS_DIR  # live lookup -- monkeypatchable in tests
...
def main(input_func=input) -> None:
    ...
    if not paths:
        print("Nenhuma proposta pendente em data/proposals/. Rode `python src/cli.py index` primeiro.")
        return
